...
from shap import KernelExplainer, DeepExplainer, GPUTreeExplainer
import matplotlib.pyplot as plt
...
class XCCShap():
    ...
    def __init__(self, model, explainer='tree', method='max', data=None, feature_perturbation="tree_path_dependent", normalize=True, topk=None, n_jobs = MAX_JOBS, nccrun=NCCRUNS, ccprogressbar=False, **kwargs):
        ...
        if explainer == 'tree':
            ...
            #self.explainer = TreeExplainer(self.model, algorithm = "auto", n_jobs = self.n_jobs, check_additivity=False, silent=True, **kwargs)
        elif explainer =='kernel':
            self.explainer = KernelExplainer(self.model.predict, data, n_jobs = self.n_jobs, silent=True, **kwargs)
            #self.explainer = KernelExplainer(self.model.predict, data, **kwargs)
        elif explainer =='deep':
            self.explainer = DeepExplainer(self.model, data, **kwargs)
        elif explainer =='gpu':
            self.explainer = GPUTreeExplainer(self.model, data, n_jobs = self.n_jobs, silent=True, **kwargs)
            if not(type(model).__name__ in ['RandomForestClassifier', 'RandomForestRegressor', 'DecisionTreeClassifier', 'DecisionTreeRegressor', 'XGBClassifier', 'XGBRegressor', 'CatBoostClassifier', 'CatBoostRegressor']):
                raise ValueError('Error: model type not supported.')
        else:
    ...
    def _get_shapmat(self, X):
        ...
        if (len(np.shape(shapmat)) > 2):
            shapmat = np.max(np.abs(shapmat),axis=0)
        else:
            shapmat = abs(shapmat)  
        if (self.normalize):
